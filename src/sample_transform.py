...
class BGForecastCF:
    ...
    def fit(self, model):
        ...
        model : TimeSeriesForecaster
            The forecasting model wrapper
        """
        self.model_ = model.model  # Store the underlying Keras model instead of the wrapper
        return self

    def predict(self, x):
        """Compute predictions using the underlying model
        ---------
        x : Variable
    ...
    def _transform_sample(
        ...
        # Create TF variables for each feature
        ...
        # uncomment for debug
        # print(
        #     f"iter: {it}, current loss: {loss}, forecast_margin_loss: {forecast_margin_loss}, weighted_steps_loss: {weighted_steps_loss}, pred:{tf.reshape(pred, [-1])} \n",
        # )
